...
- `paper-export-helper` — Paper.nu: intercepts clicks on the top-level EXPORT button on the schedule view and opens a walkthrough modal with per-app import steps (Google Calendar / Apple Calendar / Outlook), a central-time warning, and "Open <calendar>" deep links. Detection in `detection.ts` finds the button by its inner `<p>EXPORT</p>` label (paper.nu's icon+label button convention). The modal's "Download .ics" CTA walks paper.nu's own flow on the user's behalf: bypass-click EXPORT (the `allowNativeClickThrough` flag lets the capture-phase interceptor pass the event through), poll for the Download button to mount inside paper.nu's modal via `waitForDownloadButton`, click it to trigger the .ics save, then dispatch Escape to dismiss paper.nu's modal — so the user is left with just the downloaded file and our walkthrough on top. Last-selected tab persists to `chrome.storage.local` (pre-warmed on construction so the click handler can open the modal synchronously). Default ON.
